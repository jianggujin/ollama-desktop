        ...
        <template v-if="downloaderStore.list?.length">
          <div class="download-item" v-for="(item, index) in downloaderStore.list" :key="index">
            <div style="display: flex;align-items: center;">
              <div class="line-1" style="font-size: 1.1rem;width: calc(100% - 30px);">fdfdfffd</div>
            ...
            <el-progress :percentage="100" text-inside :stroke-width="20" striped striped-flow :duration="20">
              <template #default="{ percentage }">
                <div style="width: 100%;display: flex;align-items: center;">
                  <span style="margin-left: 10px;">Progressing</span>
                  <span style="margin-left: auto;margin-right: 10px;">{{ percentage }}%</span>
                </div>
              </template>
            </el-progress>
          </div>
...
onMounted(() => {
  ...
  runQuietly(() => { EventsOn('downloader', list => { downloaderStore.list = list }) })
})

onUnmounted(() => {
  runQuietly(() => { EventsOff('ollamaHeartbeat') })
  runQuietly(() => { EventsOff('downloader') })
})
...
const format = (percentage) => (percentage === 100 ? 'FullXXXXXXXXXXXXXXXXXXX' : `${percentage}%`)
